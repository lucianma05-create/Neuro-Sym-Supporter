...
class Template:
    ...
    tools: Optional[str],
        ...
        for i, message in enumerate(messages):
            ...
            if message["role"] == Role.USER:
                # 从 message 中提取 strategy 和 state
                ...
                merged_input = (
                    ...
                    f"[State]{state.strip()}"
                    f"[User Input]{content.strip()}"
                )

                # print(merged_input)

                elements += self.format_user.apply(content=merged_input, idx=str(i // 2))
...
#         if message["role"] == Role.USER:
#             # 从 message 中提取 strategy 和 state - 增强版
#             strategy = ""
#             state = ""
            
#             # 尝试多种可能的字段名称（适配您的数据格式）
#             if "strategy" in message:
#                 strategy = message["strategy"]
#             elif "cognitive_strategy" in message:
#                 strategy = message["cognitive_strategy"]
#             elif "guidance_strategy" in message:
#                 strategy = message["guidance_strategy"]
                
#             if "state" in message:
#                 state = message["state"]
#             elif "stance" in message:  # 根据您的数据集描述，可能使用"stance"字段
#                 state = message["stance"]
#             elif "cognitive_state" in message:
#                 state = message["cognitive_state"]
#             elif "guidance_state" in message:
#                 state = message["guidance_state"]
...
#             # 添加调试日志（仅在训练开始时显示一次）
#             if i == 0 and hasattr(self, "debug_logged") is False:
#                 logger.info_rank0(f"Strategy field detected: {bool(strategy)}")
#                 logger.info_rank0(f"State field detected: {bool(state)}")
#                 if strategy:
#                     logger.info_rank0(f"Sample strategy: '{strategy[:50]}{'...' if len(strategy) > 50 else ''}'")
#                 if state:
#                     logger.info_rank0(f"Sample state: '{state[:50]}{'...' if len(state) > 50 else ''}'")
#                 self.debug_logged = True

#             # 拼接为统一的 prompt 输入
#             # 即使strategy或state为空，也保留结构，避免格式混乱
#             merged_input = (
#                 f"[Strategy]\n{strategy or 'N/A'}\n\n"
#                 f"[State]\n{state or 'N/A'}\n\n"
#                 f"[User Input]\n{content}"
